...
public class UtilidadesTPV {
    ...
    public static List<Producto> listaProductos(String fichero) {
        ...
        try (Scanner datosFichero = new Scanner(new File(fichero), "UTF-8")) {
            ...
            while (datosFichero.hasNextLine()) {
                ...
                if (atributo[1].equalsIgnoreCase("poke")
                        || atributo[1].equalsIgnoreCase("wrap")
                        || atributo[1].equalsIgnoreCase("loco_moco")) {
                    lista.add(new Comida(atributo[0], CategoriasComida.valueOf(atributo[1]),
                            Double.parseDouble(atributo[2]),
                            IVA.valueOf(atributo[3]), Integer.parseInt(atributo[4])));
                }
                if (atributo[1].equalsIgnoreCase("agua")
                        || atributo[1].equalsIgnoreCase("cerveza")
                        || atributo[1].equalsIgnoreCase("refresco")) {
                    lista.add(new Bebida(atributo[0], CategoriasBebida.valueOf(atributo[1]),
                            Double.parseDouble(atributo[2]),
                            IVA.valueOf(atributo[3]), Integer.parseInt(atributo[4])));
                }
                if (atributo[1].equalsIgnoreCase("fruta")
                        || atributo[1].equalsIgnoreCase("tarta")
                        || atributo[1].equalsIgnoreCase("helado")) {
                    lista.add(new Postre(atributo[0], CategoriasPostre.valueOf(atributo[1]),
                            Double.parseDouble(atributo[2]),
                            IVA.valueOf(atributo[3]), Integer.parseInt(atributo[4])));
                }
            }
    ...
    public static List<Producto> listaTickets(String fichero) {
        ...
        try (Scanner datosFichero = new Scanner(new File(fichero), "UTF-8")) {
            ...
            while (datosFichero.hasNextLine()) {
                ...
                lista.add(new Ticket(atributo[0], CategoriasComida.valueOf(atributo[1]),
                        Double.parseDouble(atributo[2]),
                        IVA.valueOf(atributo[3]), Integer.parseInt(atributo[4])));
            }
        } catch (FileNotFoundException e) {
    ...
    public static Producto buscarProductoPorNombre(String nombre, List<Producto> listaProductos) {
        for (Producto p : listaProductos) {
            if (nombre.equalsIgnoreCase(p.getDescripcion())) {
                return p;
            }
        }
        return null;
    ...
    public static boolean seleccionarTipo(TPV tpv) {
        ...
        Object[] options = {"Comida", "Bebida", "Postre", "Volver", "Ver carrito"};
    ...
    private static void seleccionarProducto(TPV tpv, String categoria, ImageIcon icono) {
        ...
        for (Producto p : baseDatosProductos) {
            ...
            if (p instanceof Comida) {
                ...
                Comida c = (Comida) p;
                if (c.getStock() > 0 && c.getCategoria().getCATEGORIA().equals(categoria)) {
                ...
                Bebida b = (Bebida) p;
                if (b.getStock() > 0 && b.getCategoria().getCATEGORIA().equals(categoria)) {
                ...
                Postre po = (Postre) p;
                if (po.getStock() > 0 && po.getCategoria().getCATEGORIA().equals(categoria)) {
    ...
    private static void añadirACarrito(List<String> nombreProductosAMostrar, List<Producto> productosAMostrar, ImageIcon icono, TPV tpv) {
        ...
        if (opcionElegidaProducto == null) {
            ...
            Producto p = buscarProductoPorNombre(opcionElegidaProducto, productosAMostrar);
...
//        también el precio total de la compra con iva y sin iva
        ...
        //Version cesta map
        for (Map.Entry<Integer, Integer> entrada : tpv.getCarrito().getCarrito().entrySet()) {
        ...
        switch (opcionElegida) {
            ...
            case 1 -> {
                tpv.getCarrito().getCarrito().clear();
                seleccionarModo();
            }
                        ...
                        if (UtilidadesTarjeta.saldoSuficiente(digitosTarjeta, totalPagar)) {
                            ...
                            for (Map.Entry<Integer, Integer> entrada : tpv.getCarrito().getCarrito().entrySet()) {
                                for (Producto p : tpv.getMenuProductos()) {
                                    if (entrada.getKey() == p.getID()) {
                                        p.setStock(p.getStock() - entrada.getValue());
                                    }
                                }
                            }

                            //Creamos el ticket con los datos anteriores
                            Ticket t = new Ticket(new HashMap(tpv.getCarrito().getCarrito()),
                            ...
                            JOptionPane.showOptionDialog(null,
                                    t.toStringChulo(tpv), "TPV - Poke Zen", JOptionPane.DEFAULT_OPTION,
